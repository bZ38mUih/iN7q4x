...
$appRJ->response['result'].= "<!DOCTYPE html>".
    ...
    "<meta name='description' content='Создание категории'/>".
...
$appRJ->response['result'].= "</div></div><div class='input-line'><label for='catDescr'>Заголовок:</label>".
    "<input type='text' name='catDescr' value='".$Cat_rd->result['catDescr']."'><div class='field-err'>";
if(isset($catErr['catDescr'])){
    $appRJ->response['result'].= $catErr['catDescr'];
}
$appRJ->response['result'].= "</div></div><div class='input-line'><label for='prodCat_parId'>dwlCatPar_id:</label>".
    "<select name='prodCat_parId'>";
/*select options-->*/
...
$appRJ->response['result'].= $catSelect."</select></div>".
    ...
    "<label for='popFlag'>Популярная:</label><input type='checkbox' name='popFlag' ";
